-- Filter timeline_events to show only relevant events (upcoming + recent past)
-- This prevents the timeline from being overwhelmed with all historical events
...
with regatta_events as (
  ...
  where r.start_date is not null
    -- Only show events from last 7 days or future events
...
clinical_events as (
  ...
  where cs.scheduled_start is not null
    -- Only show events from last 7 days or future events
    and cs.scheduled_start >= now() - interval '7 days'
)
