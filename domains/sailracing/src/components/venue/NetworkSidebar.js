...
import { View, StyleSheet, TouchableOpacity, ScrollView, Animated, Platform, ActivityIndicator, TextInput, } from 'react-native';
import { ThemedText } from '@betterat/ui/components/themed-text';
import { Ionicons } from '@expo/vector-icons';
import { SailingNetworkService } from '@/services/SailingNetworkService';
import { supabase } from '@betterat/core/database';
import { getShadowStyle } from '@betterat/core/lib/styles/shadow';
export function NetworkSidebar({ isCollapsed, onToggleCollapse, onPlacePress, }) {
    ...
    const performSearch = async () => {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Search timed out after 5 seconds')), 5000));
    ...
    const handleSave = async (place) => {
        try {
            await SailingNetworkService.saveToNetwork(place.id, place.type, place.location);
            await fetchSavedPlaces();
            // Update search results to reflect saved status
            setSearchResults(prev => prev.map(p => p.id === place.id ? { ...p, isSaved: true } : p));
        }
        catch (error) {
    ...
    const handleUnsave = async (place) => {
        try {
            ...
            setSearchResults(prev => prev.map(p => p.id === place.id ? { ...p, isSaved: false } : p));
        }
        catch (error) {
    ...
    const renderPlace = (place) => (<View key={place.id} style={styles.placeCard}>
      ...
      <View style={styles.placeActions}>
        <TouchableOpacity style={styles.detailsButton} onPress={() => onPlacePress(place)}>
          <Ionicons name="information-circle-outline" size={20} color="#666"/>
        </TouchableOpacity>
        {place.isSaved ? (<TouchableOpacity style={styles.savedButton} onPress={() => handleUnsave(place)}>
            <Ionicons name="bookmark" size={20} color="#007AFF"/>
          </TouchableOpacity>) : (<TouchableOpacity style={styles.saveButton} onPress={() => handleSave(place)}>
    ...
    return (<>
      <Animated.View style={[
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search Hong Kong, RHKYC, North Sails..." placeholderTextColor="#999" value={searchQuery} onChangeText={setSearchQuery} autoCorrect={false} autoCapitalize="none"/>
          {searchQuery.length > 0 && (<TouchableOpacity onPress={() => setSearchQuery('')} style={styles.clearButton}>
        ...
        // Search Results
        <>
              {isSearching ? (<View style={styles.loadingContainer}>
                  <ActivityIndicator size="large" color="#007AFF"/>
                </View>) : searchResults.length === 0 ? (<View style={styles.emptyState}>
        ...
        <>
              {isLoadingSaved ? (<View style={styles.loadingContainer}>
                  <ActivityIndicator size="large" color="#007AFF"/>
                </View>) : savedPlaces.length === 0 ? (<View style={styles.emptyState}>
      ...
      <TouchableOpacity style={[styles.toggleButton, isCollapsed && styles.toggleButtonCollapsed]} onPress={onToggleCollapse}>
        <Ionicons name={isCollapsed ? 'chevron-forward' : 'chevron-back'} size={20} color="#fff"/>
      </TouchableOpacity>
    </>);
}
