...
export class DocumentStorageService {
  ...
  /**
   * Upload an existing File/Blob (web) without invoking the picker.
   * Primarily used when another service already has a File handle (e.g., drag/drop).
   */
  async uploadExistingDocument(userId: string, file: File | Blob, filename?: string): Promise<UploadResult> {
    if (typeof window === 'undefined') {
      return { success: false, error: 'Direct file upload is only supported in web environments' };
    }

    const objectUrl = URL.createObjectURL(file);
    const name = filename ?? ('name' in file ? (file as File).name ?? 'document' : 'document');
    const size: number = 'size' in file && typeof (file as File).size === 'number' ? (file as File).size : 0;
    const mimeType = 'type' in file ? (file as File).type ?? 'application/octet-stream' : 'application/octet-stream';

    const lastModified =
      'lastModified' in file && typeof (file as File).lastModified === 'number'
    ...
    try {
      return await this.uploadDocument(userId, asset);
    } finally {
