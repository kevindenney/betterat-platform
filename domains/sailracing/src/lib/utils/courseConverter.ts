...
interface RaceMarkWithMetadata extends RaceMark {
  extractionMetadata?: {
    confidence?: number;
    source?: string;
    extractedAt?: string;
...
export function convertCourseToMarks(courseExtraction: RaceCourseExtraction): RaceMark[] {
  const marks: RaceMarkWithMetadata[] = [];
  let markIdCounter = 1;
  ...
  courseExtraction.marks.forEach((extractedMark, index) => {
    ...
    if (extractedMark.position?.latitude && extractedMark.position?.longitude) {
      const mark: RaceMarkWithMetadata = {
  ...
  const meta = courseExtraction.extractionMetadata;
  if (meta) {
    marks.forEach(mark => {
      mark.extractionMetadata = {
        confidence: meta.overallConfidence,
        source: meta.source,
        extractedAt: typeof meta.extractedAt === 'string'
          ? meta.extractedAt
          : meta.extractedAt?.toISOString?.()
      };
    });
  }
  return marks;
