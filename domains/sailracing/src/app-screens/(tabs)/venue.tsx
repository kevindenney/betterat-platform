...
export default function VenueIntelligenceScreen() {
  ...
  const [sidebarSavedPlaces, setSidebarSavedPlaces] = useState<NetworkPlace[]>([]);
  const [sidebarSearchResults, setSidebarSearchResults] = useState<NetworkPlace[]>([]);
  ...
  const toSidebarPlace = useCallback((place: NetworkPlace): NetworkPlace => ({
    id: place.id,
    name: place.name,
  ...
  const handleSidebarSaveToggle = useCallback(async (place: NetworkPlace, nextSaved: boolean) => {
    try {
      if (nextSaved) {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      try {
        ...
        const results: NetworkPlace[] = (data || []).map((venue: any) => ({
          id: venue.id,
          name: venue.name,
