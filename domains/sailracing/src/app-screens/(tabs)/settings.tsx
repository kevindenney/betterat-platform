...
import { ClubAiAssistant } from '@betterat/ui/components/ai/ClubAiAssistant';

export default function SettingsScreen() {
  const { user, userProfile, clubProfile, signOut, isDemoSession } = useAuth();
  const [claimVisible, setClaimVisible] = useState(false);
  const [claimPassword, setClaimPassword] = useState('');
  const [claimPasswordConfirm, setClaimPasswordConfirm] = useState('');
  const [claimLoading, setClaimLoading] = useState(false);

  const onboardingStep = (userProfile as any)?.onboarding_step as string | undefined;
  const isDemoProfile = useMemo(
    () => isDemoSession || (onboardingStep ?? '').toString().startsWith('demo'),
    [isDemoSession, onboardingStep]
  );
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSubtitle}>
              Set a password so you can sign back in and keep the progress you&rsquo;ve made in demo mode.
            </Text>
