...
import type { RaceCourseExtraction, TacticalRecommendation } from '@betterat/core/lib/types/ai-knowledge';
import type { CourseMark } from '@betterat/core/types/raceEvents';
import { RaceMapView } from '@betterat/ui/components/race-strategy/RaceMapView';
import { QuickDrawMode } from '@betterat/ui/components/race-strategy/QuickDrawMode';
import { VictoryLine, VictoryChart, VictoryTheme, VictoryAxis, VictoryArea } from 'victory';
import { RaceDetailsView, DocumentList, type Document, CourseVisualization, CourseSetupPrompt, CourseSelector } from '@betterat/ui/components/races';
...
const TAB_TYPES: TabType[] = ['overview', 'details', 'strategy', 'documents', 'crew', 'equipment', 'tracks', 'results'];

const isTabType = (value?: string | null): value is TabType =>
  typeof value === 'string' && TAB_TYPES.includes(value as TabType);

const coerceParam = (value?: string | string[] | null): string | undefined =>
...
function LegacyRaceDetailScreen({
  id,
  tab,
  action,
  courseId,
  ...
  const renderOverview = () => (
    <View style={styles.tabContent}>
      ...
      {(() => {
        ...
        const visualizationMarks: CourseMark[] = docWithExtraction.extraction.marks.reduce(
          (acc: CourseMark[], mark, index) => {
...
export default function RaceDetailRoute() {
  const params = useLocalSearchParams<RaceDetailRouteParams>();
  const idParam = coerceParam(params.id);
  const tabParam = coerceParam(params.tab);
  const actionParam = coerceParam(params.action);
  const courseIdParam = coerceParam(params.courseId);

  const normalizedTab = isTabType(tabParam) ? tabParam : undefined;
  const normalizedAction: RaceDetailRouteParams['action'] | undefined =
    actionParam === 'quickdraw' || actionParam === 'template' ? actionParam : undefined;
