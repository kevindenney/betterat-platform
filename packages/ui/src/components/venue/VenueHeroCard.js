...
export function VenueHeroCard({ venueName, country, region, distanceLabel, travelTip, windSummary, onNavigate, onSave, isSaved = false, latitude, longitude, }) {
    const openMaps = () => {
    ...
    return (<View style={styles.container}>
      <View style={styles.titleRow}>
        ...
        {!!country && (<View style={styles.countryPill}>
      ...
      {!!distanceLabel && (<View style={styles.infoRow}>
          <Ionicons name="pin-outline" size={16} color="#2563eb"/>
          <ThemedText style={styles.infoText}>{distanceLabel}</ThemedText>
        </View>)}

      {!!windSummary && (<View style={styles.infoRow}>
          <Ionicons name="compass-outline" size={16} color="#2563eb"/>
          <ThemedText style={styles.infoText}>{windSummary}</ThemedText>
        </View>)}

      {!!travelTip && (<View style={styles.tipBubble}>
          <ThemedText style={styles.tipText}>{travelTip}</ThemedText>
        </View>)}

      <View style={styles.actionRow}>
        <TouchableOpacity style={styles.primaryButton} onPress={onNavigate ?? openMaps} disabled={!latitude || !longitude}>
          <Ionicons name="navigate" size={18} color="#fff"/>
          <ThemedText style={styles.primaryButtonText}>Navigate</ThemedText>
        </TouchableOpacity>

        {!!onSave && (<TouchableOpacity style={[styles.secondaryButton, isSaved && styles.secondaryButtonActive]} onPress={onSave}>
            <Ionicons name={isSaved ? 'bookmark' : 'bookmark-outline'} size={18} color={isSaved ? '#2563eb' : '#1f2937'}/>
            <ThemedText style={[
