...
export function MapControls({ onToggle3D, onToggleLayers, onSearchNearby, onSettings, onToggleSavedVenues, is3DEnabled = false, areLayersVisible = true, showOnlySavedVenues = false, savedVenuesCount = 0, layers: externalLayers, onLayersChange, }) {
    ...
    return (<View style={styles.container}>
      <View style={styles.controlBar}>
        {onToggle3D && (<TouchableOpacity style={[styles.controlButton, is3DEnabled && styles.controlButtonActive]} onPress={onToggle3D}>
            <Ionicons name="cube-outline" size={20} color={is3DEnabled ? '#007AFF' : '#333'}/>
            <ThemedText style={[styles.buttonText, is3DEnabled && styles.buttonTextActive]}>
              3D
            </ThemedText>
          </TouchableOpacity>)}

        {onToggleLayers && (<TouchableOpacity style={[styles.controlButton, areLayersVisible && styles.controlButtonActive]} onPress={() => {
                setLayerMenuOpen(!layerMenuOpen);
                onToggleLayers();
            }}>
            <Ionicons name="layers-outline" size={20} color={areLayersVisible ? '#007AFF' : '#333'}/>
            <ThemedText style={[styles.buttonText, areLayersVisible && styles.buttonTextActive]}>
              Layers
            </ThemedText>
          </TouchableOpacity>)}

        {onToggleSavedVenues && (<TouchableOpacity style={[styles.controlButton, showOnlySavedVenues && styles.controlButtonActive]} onPress={onToggleSavedVenues}>
            <Ionicons name={showOnlySavedVenues ? 'bookmark' : 'bookmark-outline'} size={20} color={showOnlySavedVenues ? '#007AFF' : '#333'}/>
            <ThemedText style={[styles.buttonText, showOnlySavedVenues && styles.buttonTextActive]}>
              {`Saved${savedVenuesCount > 0 ? ` (${savedVenuesCount})` : ''}`}
            </ThemedText>
          </TouchableOpacity>)}

        {onSearchNearby && (<TouchableOpacity style={styles.controlButton} onPress={onSearchNearby}>
            <Ionicons name="navigate-outline" size={20} color="#333"/>
            <ThemedText style={styles.buttonText}>Nearby</ThemedText>
          </TouchableOpacity>)}

        {onSettings && (<TouchableOpacity style={styles.controlButton} onPress={onSettings}>
            <Ionicons name="settings-outline" size={20} color="#333"/>
          </TouchableOpacity>)}
      </View>

      {layerMenuOpen && (<View style={styles.layerMenu}>
