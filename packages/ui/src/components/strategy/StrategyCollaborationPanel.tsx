...
const defaultTheme: StrategyCollaborationPanelTheme = {
  background: '#FFFBF5',
  border: '#E4D7C6',
  heading: '#3F372D',
  text: '#5E5549',
...
export const StrategyCollaborationPanel: React.FC<StrategyCollaborationPanelProps> = ({
  heading,
  description,
  connectionLabel,
  ...
  const appliedTheme: StrategyCollaborationPanelTheme = { ...defaultTheme, ...(theme || {}) };

  return (
    <View
      style={[styles.container, { backgroundColor: appliedTheme.background, borderColor: appliedTheme.border }]}
    ...
    >
      <View style={styles.headerRow}>
        ...
        <TouchableOpacity
          onPress={onConnect}
          style={[styles.connectButton, { backgroundColor: appliedTheme.accent }]}
      ...
      <View style={styles.collaboratorGrid}>
        {collaborators.map((collaborator, index) => (
          ...
          >
            ...
            {collaborator.planFocus ? (
              <Text style={[styles.collaboratorFocus, { color: appliedTheme.text }]}>Focus: {collaborator.planFocus}</Text>
            ) : null}
            {collaborator.status ? (
              <View style={[styles.statusPill, { backgroundColor: appliedTheme.pill }]}
              >
      ...
      <View style={styles.topicStack}>
        {topics.map((topic) => (
          <View
            key={topic.id}
            style={[styles.topicCard, { borderColor: appliedTheme.border }]}
          >
              ...
              {topic.tags?.length ? (
                <View style={styles.tagRow}>
                  {topic.tags.map((tag) => (
                    <View
                      key={`${topic.id}-${tag}`}
                      style={[styles.tagPill, { borderColor: appliedTheme.pill }]}
                    >
