...
import { ChecklistItem, DashboardSection, DataTable, InfoStack, MetricCard, StatusBadge, } from './base';
import { TimelineStrip } from './TimelineStrip';
const DEFAULT_CREW_COLUMNS = [
    { key: 'role', label: 'Role' },
...
const DEFAULT_FINISHING_COLUMNS = [
...
const MetricGrid = ({ data }) => (<View style={styles.metricGrid}>
    {data.map((metric) => (<MetricCard key={metric.label} {...metric}/>))}
  </View>);
const StageRail = ({ stages }) => (<View style={styles.stageRail}>
    {stages.map((stage) => (<View key={stage.label} style={styles.stageCard}>
...
const TimelineCard = ({ label, summary, notes }) => (<View style={styles.timelineCard}>
    <Text style={styles.timelineLabel}>{label}</Text>
    <Text style={styles.timelineSummary}>{summary}</Text>
    <View style={styles.timelineList}>
      {notes.map((note) => (<View key={`${label}-${note}`} style={styles.timelineListRow}>
          <View style={styles.timelineBullet}/>
          <Text style={styles.timelineNote}>{note}</Text>
        </View>))}
    </View>
  </View>);
const MapPanel = ({ title, subtitle, badge, placeholder }) => (<View style={styles.mapPanel}>
...
const ActionBar = ({ actions }) => (<View style={styles.actionBar}>
    {actions.map((action) => (<TouchableOpacity key={action} style={styles.actionChip}>
        <Text style={styles.actionChipText}>{action}</Text>
      </TouchableOpacity>))}
  </View>);
export const ReadinessDashboard = ({ loading = false, error, hero, stages, actionLabels, kpis, crewRows, logisticsChecklist, timelineItems, selectedTimeline, onSelectTimeline, weatherRows, analyticsCards, aiNotes, documents, finishing, protests, mapMeta = {
    ...
    return (<SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        {loading ? (<View style={styles.syncBanner}>
        ...
        <DashboardSection title={hero.title} description={hero.description} actions={<StatusBadge label={copyConfig.heroBadgeLabel} value={hero.countdownLabel} tone="info"/>}>
        ...
        <DashboardSection title={copyConfig.crew.title} description={copyConfig.crew.description}>
          <DataTable columns={copyConfig.crew.columns ?? DEFAULT_CREW_COLUMNS} rows={crewRows.map((row) => ({
            role: row.role,
            sailor: row.sailor,
            call: row.call,
        ...
        <DashboardSection title={copyConfig.map.title} description={copyConfig.map.description}>
          <MapPanel title={mapMeta.title} subtitle={mapMeta.subtitle} badge={mapMeta.badge} placeholder={copyConfig.mapPlaceholder}/>
        </DashboardSection>
        ...
        <DashboardSection title={copyConfig.timeline.title} description={copyConfig.timeline.description}>
          ...
          <View style={styles.timelineTabs}>
            {timelineItems.map((timeline) => (<TouchableOpacity key={timeline.id} style={[styles.timelineTab, selectedTimeline === timeline.label && styles.timelineTabActive]} onPress={() => onSelectTimeline(timeline.label)}>
                <Text style={[styles.timelineTabText, selectedTimeline === timeline.label && styles.timelineTabTextActive]}>
                  {timeline.label}
                </Text>
              </TouchableOpacity>))}
          </View>
          {timelineActive.map((item) => (<TimelineCard key={item.id} {...item}/>))}
        </DashboardSection>

        <DashboardSection title={copyConfig.logistics.title} description={copyConfig.logistics.description}>
          {logisticsChecklist.map((item) => (<ChecklistItem key={item.label} {...item}/>))}
        </DashboardSection>

        <DashboardSection title={copyConfig.weather.title} description={copyConfig.weather.description}>
          <View style={styles.weatherGrid}>
            {weatherRows.map((row) => (<View key={row.label} style={styles.weatherCard}>
        ...
        <DashboardSection title={copyConfig.ai.title} description={copyConfig.ai.description}>
          <View style={styles.aiCard}>
            {aiNotes.map((note) => (<View key={note} style={styles.aiNote}>
                <View style={styles.timelineBullet}/>
        ...
        <DashboardSection title={copyConfig.protests.title} description={copyConfig.protests.description}>
          {protests.map((item) => (<View key={item.label} style={styles.protestRow}>
        ...
        <DashboardSection title={copyConfig.documents.title} description={copyConfig.documents.description}>
          <DataTable columns={copyConfig.documents.columns ?? DEFAULT_DOCUMENT_COLUMNS} rows={documents.map((doc) => ({
            type: doc.type,
            owner: doc.owner,
            updated: doc.updated,
            status: doc.status,
        ...
        <DashboardSection title={copyConfig.finishing.title} description={copyConfig.finishing.description}>
          <DataTable columns={copyConfig.finishing.columns ?? DEFAULT_FINISHING_COLUMNS} rows={finishing.map((row) => ({
            place: row.place,
            boat: row.boat,
            delta: row.delta,
            points: row.points,
