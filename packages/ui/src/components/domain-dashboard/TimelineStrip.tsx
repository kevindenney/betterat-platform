...
import type { TimelineItem } from './ReadinessDashboard';
...
export const TimelineStrip: React.FC<TimelineStripProps> = ({ events, currentTime = new Date() }) => {
  const meta = useMemo(() => {
    ...
    const timed = parsed.filter((event) => event.timestamp !== null) as Array<typeof parsed[number]>;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {meta.nowPercent !== null && (
          <View style={[styles.nowMarker, { left: `${meta.nowPercent * 100}%` }] }>
            <View style={styles.nowLine} />
            <Text style={styles.nowLabel}>Now</Text>
          </View>
        )}
        {meta.nodes.map((event) => (
          <View key={event.id} style={[styles.node, { left: `${event.percent * 100}%` }]}>
