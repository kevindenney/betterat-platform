...
export const DomainSwitcher = ({ activeDomains, currentDomainId, onSwitch, domainMetadata = {}, style, }) => {
    ...
    const getDomainMeta = (domainId) => {
        return (domainMetadata[domainId] || {
            id: domainId,
            name: domainId,
            icon: '📦',
    ...
    const renderDomainCard = ({ item }) => {
        ...
        <DomainCard domain={domain} isActive={isActive} onPress={() => handleDomainPress(item)}/>
      </View>);
    };
    return (<View style={style}>
      ...
      <Modal visible={modalVisible} onClose={() => setModalVisible(false)} title="Switch Domain">
        <FlatList data={activeDomains} renderItem={renderDomainCard} keyExtractor={(item) => item} numColumns={1} contentContainerStyle={styles.gridContainer} showsVerticalScrollIndicator={false} ListEmptyComponent={<View style={styles.emptyContainer}>
