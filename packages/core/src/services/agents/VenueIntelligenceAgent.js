...
import { venueIntelligenceService } from '../VenueIntelligenceService';
import { z } from 'zod';
            ...
            execute: async (input) => {
                try {
                    ...
                    return {
                        ...
                        alternatives: venues.slice(1, 4).map((v) => ({
                            venueId: v.id,
                            name: v.name,
...
Critical for race safety and strategy planning.`,
     ...
     */
    createApplyCulturalSettingsTool() {
        return {
...
Critical for sailors who lose connectivity on the water.`,
     ...
     */
    async switchVenueByGPS(coordinates) {
        return this.run({
     ...
     */
    async switchVenueBySelection(venueId) {
        return this.run({
     ...
     */
    async refreshVenueWeather(venueId) {
        return this.run({
     ...
     */
    async getCachedInsights(venueId, userId) {
        try {
     ...
     */
    async cacheInsights(venueId, userId, insights, metadata = {}) {
        try {
     ...
     */
    async invalidateCache(venueId, userId) {
        try {
            await supabase
                .from('venue_intelligence_cache')
                .delete()
     ...
     */
    async analyzeVenue(venueId, userId, forceRefresh = false) {
        try {
            // Check cache first (if userId provided and not forcing refresh)
            if (userId && !forceRefresh) {
                const cached = await this.getCachedInsights(venueId, userId);
                if (cached) {
                    const ageHours = Math.floor((Date.now() - new Date(cached.generatedAt).getTime()) / (1000 * 60 * 60));
                    return {
                        success: true,
...
Structure your response as actionable recommendations for a sailor new to this venue.`,
            ...
            if (userId) {
                await this.cacheInsights(venueId, userId, insights, {
     ...
     */
    extractSection(response, sectionKeyword) {
