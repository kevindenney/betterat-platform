...
function normalizeRaceTime(time) {
    if (!time)
        return null;
...
function buildRaceDate(raceDate, raceTime) {
    if (!raceDate)
        return null;
    ...
    if (normalizedTime) {
        ...
        const shouldOverride = !hasExplicitTime ||
            isoIsMidnight ||
...
export function calculateCountdown(raceDate, raceTime) {
    // Validate input
    if (!raceDate) {
 ...
 */
export function getMockDataForUser(userId) {
    return {
        races: MOCK_RACES,
        boats: MOCK_BOATS,
 ...
 */
export function getRaceWithLinks(raceId) {
 ...
 */
export function getRacesForBoat(boatId) {
    return MOCK_RACES.filter((r) => r.boatId === boatId);
}
/**
 * Phase 6: Get all races using a specific course
 */
export function getRacesForCourse(courseId) {
    return MOCK_RACES.filter((r) => r.courseId === courseId);
}
// Legacy lowercase exports for backward compatibility with older imports
