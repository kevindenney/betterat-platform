 ...
 */
const BUILT_IN_SKILL_DEFINITIONS = {
...
function resolveBuiltInSkill(skillName) {
...
function getPreconfiguredSkillId(skillKey) {
...
export class SkillManagementService {
    constructor() {
        ...
        this.remoteSkillDisableReason = null;
        this.remoteSkillNoticeLogged = false;
        this.loggedSkipForSkill = new Set();
        this.loggedPreconfiguredSkill = new Set();
        ...
        const disableViaEnv = (process.env.EXPO_PUBLIC_DISABLE_REMOTE_SKILLS ?? '').toLowerCase() === 'true';
        if (disableViaEnv) {
     ...
     */
    async callSkillsProxy(action, params = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 404 ||
                    ...
                    this.disableRemoteSkills(`Skill proxy returned server error (HTTP ${response.status}). Check Supabase Edge Function logs.`);
                }
     ...
     */
    async uploadSkill(name, description, content) {
     ...
     */
    slugifyDisplayTitle(displayTitle) {
        return displayTitle
            .toLowerCase()
            .replace(/\s+/g, '-')
            .replace(/[^a-z0-9-]/g, '');
    ...
    async listSkills() {
        ...
        try {
            ...
            if (skills.length > 0) {
                const skillNames = skills.map((s) => s.name || s.display_title).join(', ');
    ...
    async initializeSkillInternal(skillKey) {
        await this.ensureInitialized();
        ...
        if (preconfiguredId) {
            if (!this.loggedPreconfiguredSkill.has(skillKey)) {
                ...
                console.debug(`Using preconfigured skill ID for '${skillKey}' (${preconfiguredId}) - skipping remote initialization`);
            }
        ...
        try {
            ...
            const skillId = await this.uploadSkill(skillKey, definition.description, skillContent);
            if (skillId) {
     ...
     */
    static getBuiltInSkillDefinition(skillName) {
        const resolved = resolveBuiltInSkill(skillName);
