...
import { z } from 'zod';
import { AgentTool } from '../agents/base';
import { supabase } from '../supabase';

// Type for external race results from the database
interface ExternalRaceResult {
  regatta_name: string;
  race_date: string;
...
I'll save this sail number so we can track future results! What's the owner/skipper name for this boat?`,
        ...
        if (externalResults && externalResults.length > 0) {
          const resultsToImport = externalResults.map((result: ExternalRaceResult) => ({
          ...
          const ownerNames = externalResults
            .map((r: ExternalRaceResult) => r.owner_name || r.helm_name || r.skipper_name)
            .filter(Boolean);

          if (ownerNames.length > 0) {
            // Most common name is likely the owner
            const nameFreq: Record<string, number> = {};
            ownerNames.forEach((name: string | undefined) => {
          ...
          const clubs = externalResults
            .map((r: ExternalRaceResult) => r.yacht_club || r.club_name)
            .filter(Boolean);
          if (clubs.length > 0) {
        ...
        const wins = externalResults.filter((r: ExternalRaceResult) => r.finish_position === 1).length;
        const podiums = externalResults.filter((r: ExternalRaceResult) => r.finish_position <= 3).length;
        const avgFinish =
          externalResults.reduce((sum: number, r: ExternalRaceResult) => sum + (r.finish_position || 0), 0) /
          resultsCount;
