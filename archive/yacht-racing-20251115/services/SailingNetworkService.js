...
export class SailingNetworkService {
     ...
     */
    static async getMyNetwork(locationName) {
        try {
            // Timeout after 5 seconds to prevent indefinite hanging
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Request timed out after 5 seconds')), 5000));
     ...
     */
    static async discoverInLocation(locationName, filters) {
        try {
            ...
            const savedVenuesPromise = supabase
                ...
                .select('venue_id')
                .eq('user_id', user.id);
            const { data: savedVenues } = await Promise.race([savedVenuesPromise, timeoutPromise]);
     ...
     */
    static async saveToNetwork(placeId, type, location, options) {
     ...
     */
    static async removeFromNetwork(placeId) {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            throw new Error('User not authenticated');
        const { error } = await supabase
     ...
     */
    static async getCurrentLocationContext() {
        try {
            // Timeout after 5 seconds
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Request timed out after 5 seconds')), 5000));
            const authPromise = supabase.auth.getUser();
     ...
     */
    static async setLocationContext(locationName, locationRegion, coordinates, detectionMethod = 'manual') {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            throw new Error('User not authenticated');
     ...
     */
    static async autoSave(placeId, placeName, type, location, coordinates, isHome = false) {
        try {
            // Save to network
            ...
            if (isHome) {
                await this.setLocationContext(location.name, location.region, coordinates, 'manual');
            }
        }
        catch (error) {
     ...
     */
    static async isInNetwork(placeId) {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            return false;
     ...
     */
    static async updateNetworkPlace(placeId, updates) {
        const { data: { user } } = await supabase.auth.getUser();
        if (!user)
            throw new Error('User not authenticated');
        // If setting as home venue, unset any existing home venue
