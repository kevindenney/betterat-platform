...
import type {
  ...
  AnalysisResponse,
  DocumentAnalysis
} from '@betterat/core/lib/types/ai-knowledge';
...
let missingAnthropicWarningShown = false;
...
export class RaceStrategyEngine {
  ...
  constructor() {
    ...
    if (!this.hasValidApiKey) {
      if (!missingAnthropicWarningShown) {
        ...
        missingAnthropicWarningShown = true;
      }
    } else {
  ...
  private async initializeSkill(): Promise<void> {
    try {
      const hasInitializer = typeof skillManagementService?.initializeRaceStrategySkill === 'function';
      const skillId = hasInitializer
