...
export function SavedNetworkSection({ locationName, savedPlaces, onPlacePress, onRemovePlace, }) {
    ...
    return (<View style={styles.container}>
      ...
      {placesInCurrentLocation.length > 0 && (<View style={styles.section}>
          <ThemedText style={styles.sectionTitle}>
            🌟 MY NETWORK{locationName ? ` IN ${locationName.toUpperCase()}` : ''}
          </ThemedText>
          {placesInCurrentLocation.map((place) => (<TouchableOpacity key={place.id} style={[
                    styles.placeCard,
              ...
              <View style={styles.placeInfo}>
                <View style={styles.placeHeader}>
                  ...
                  {place.isHomeVenue && (<View style={styles.homeBadge}>
                ...
                {place.notes && (<ThemedText style={styles.placeNotes} numberOfLines={1}>
                    {place.notes}
                  </ThemedText>)}
              </View>
      ...
      {Object.keys(groupedOtherLocations).length > 0 && (<View style={styles.section}>
          <TouchableOpacity style={styles.otherLocationsHeader} onPress={() => setShowOtherLocations(!showOtherLocations)}>
            <ThemedText style={styles.otherLocationsTitle}>
              Other Locations ({Object.keys(groupedOtherLocations).length})
            </ThemedText>
            <Ionicons name={showOtherLocations ? 'chevron-up' : 'chevron-down'} size={18} color="#666"/>
          </TouchableOpacity>

          {showOtherLocations && (<View style={styles.otherLocationsList}>
              {Object.entries(groupedOtherLocations).map(([locationName, places]) => (<View key={locationName} style={styles.otherLocationGroup}>
                  <ThemedText style={styles.otherLocationName}>
                    📍 {locationName} ({places.length} saved)
                  </ThemedText>
                  {places.slice(0, 2).map((place) => (<TouchableOpacity key={place.id} style={styles.compactPlaceCard} onPress={() => onPlacePress(place)}>
                  ...
                  {places.length > 2 && (<ThemedText style={styles.moreCount}>
