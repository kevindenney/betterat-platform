...
export function RaceRegistrationForm({
  ...
  const loadRegattaDetails = useCallback(async () => {
    try {
  ...
  const loadCrewMembers = useCallback(
    async (boatId: string) => {
      try {
        ...
        const raceDate =
          regattaDetails?.start_date || new Date().toISOString().split('T')[0];
  ...
  }, [selectedBoatId, loadCrewMembers]);

  // Check crew availability
  const checkCrewAvailability = useCallback(async () => {
    try {
  ...
  }, [selectedCrewIds, regattaId]);

  useEffect(() => {
    if (selectedCrewIds.length > 0) {
      checkCrewAvailability();
    }
  }, [selectedCrewIds, checkCrewAvailability]);
