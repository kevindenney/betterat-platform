...
import { sailorBoatService, type CreateBoatInput } from '@betterat/core';
import { DRAGON_HULL_MAKER_NAMES } from '@betterat/core/constants/boatEquipment';
...
export default function AddBoatScreen() {
  ...
  const loadBoatClasses = async () => {
    ...
    try {
      ...
      const classes = await sailorBoatService.listBoatClasses();
      logger.debug('Loaded boat classes from shared service:', classes.length);
      setBoatClasses(classes.length ? classes : FALLBACK_BOAT_CLASSES);
    } catch (error) {
