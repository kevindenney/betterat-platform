...
import { CourseBuilder, type CourseDraft } from '@betterat/ui/components/courses';
import { CourseLibraryService } from '@/services/CourseLibraryService';
import { useAuth } from '@betterat/core/providers/AuthProvider';
import type { CourseType, Mark, MarkType, RaceCourse } from '@betterat/core/types/courses';
import { getRacesForCourse, MOCK_COURSES, type MockCourse } from '@betterat/core/constants/mockData';
...
export default function CoursesScreen() {
  ...
  const profileClubId = (userProfile as { club_id?: string } | null)?.club_id ?? null;
  ...
  const loadCourses = useCallback(async () => {
    ...
    try {
      const remoteCourses = await CourseLibraryService.fetchCourses({
        clubId: profileClubId ?? undefined,
      });
  ...
  }, [fallbackCourses, user?.id, profileClubId]);
  ...
  const handleSaveCourse = async (draft: CourseDraft) => {
    ...
    setSaving(true);
    const scope = profileClubId ? 'club' : 'personal';
    const clubId = profileClubId ?? undefined;
      ...
      <View style={styles.modalOverlay}>
        <View style={styles.modalCard}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>Assign &ldquo;{course.name}&rdquo;</Text>
