...
import {
    ...
    WindWeatherCard,
    RigTuningCard,
} from '@betterat/ui/components/race-detail';
import type { RaceDocument as RaceDocumentsCardDocument } from '@betterat/ui/components/race-detail/RaceDocumentsCard';
import { PlanModeLayout } from '@betterat/ui/components/races/modes';
import { PerformanceMetrics, calculatePerformanceMetrics } from '@betterat/ui/components/races/PerformanceMetrics';
import { SplitTimesAnalysis } from '@betterat/ui/components/races/SplitTimesAnalysis';
import { TacticalInsights } from '@betterat/ui/components/races/TacticalInsights';
import { AIPatternDetection } from '@betterat/ui/components/races/debrief/AIPatternDetection';
import { LiveRaceTimer } from '@betterat/ui/components/races/LiveRaceTimer';
import { LivePositionTracker } from '@betterat/ui/components/races/LivePositionTracker';
import { TacticalDataOverlay, TacticalCalculations } from '@betterat/ui/components/races/TacticalDataOverlay';
import { QuickActionDrawer } from '@betterat/ui/components/races/QuickActionDrawer';
import { OnWaterTrackingView } from '@betterat/ui/components/races/OnWaterTrackingView';
import { PhaseHeader, PhaseSkillButtons, PrimaryAICoach } from '@betterat/ui/components/races';
import { PreStartConsole } from '@betterat/ui/components/racing-console/PreStartConsole';
import { PlanModeContent } from '@betterat/ui/components/races/plan';
import { CalendarImportFlow } from '@betterat/ui/components/races/CalendarImportFlow';
import { DemoRaceDetail } from '@betterat/ui/components/races/DemoRaceDetail';
import { PostRaceInterview } from '@betterat/ui/components/races/PostRaceInterview';
import { FleetPostRaceInsights } from '@betterat/ui/components/races/FleetPostRaceInsights';
import { RaceCard } from '@betterat/ui/components/races/RaceCard';
import { SmartRaceCoach } from '@betterat/ui/components/coaching/SmartRaceCoach';
import { QuickSkillButtons } from '@betterat/ui/components/coaching/QuickSkillButtons';
import { CoachSelectionModal } from '@betterat/ui/components/coaching/CoachSelectionModal';
import { AccessibleTouchTarget } from '@betterat/ui/components/ui/AccessibleTouchTarget';
import { Button, ButtonIcon, ButtonText } from '@betterat/ui/components/ui/button';
import { Card } from '@betterat/ui/components/ui/card';
import { ErrorMessage } from '@betterat/ui/components/ui/error';
import { DashboardSkeleton } from '@betterat/ui/components/ui/loading';
import { OfflineIndicator } from '@betterat/ui/components/ui/OfflineIndicator';
import { MOCK_RACES, calculateCountdown } from '@betterat/core/constants/mockData';
import { useDashboardData } from '@betterat/core/hooks/useData';
import { useOffline } from '@betterat/core/hooks/useOffline';
import { useLiveRaces } from '@betterat/core/hooks/useRaceResults';
import { useRaceWeather } from '@betterat/core/hooks/useRaceWeather';
import { useEnrichedRaces } from '@betterat/core/hooks/useEnrichedRaces';
import { useVenueDetection } from '@betterat/core/hooks/useVenueDetection';
...
import { useRaceTuningRecommendation } from '@betterat/core/hooks/useRaceTuningRecommendation';
import { useRacePreparation } from '@betterat/core/hooks/useRacePreparation';
import { useRaceBriefSync } from '@betterat/core/hooks/ai/useRaceBriefSync';
import type { Mark } from '@betterat/core/types/courses';
import { useRaceConditions } from '@betterat/core/stores/raceConditionsStore';
import { useRacePhaseDetection } from '@betterat/core/hooks/useRacePhaseDetection';
import type { Course } from '@betterat/core/stores/raceConditionsStore';
import type { GPSPoint } from '@/services/gpsService';
...
const DOCUMENT_TYPE_OPTIONS: { label: string; description: string; value: RaceDocumentType }[] = [
  {
...
function pickNumeric(values: unknown[]): number | null {
...
export default function RacesScreen() {
  ...
  useEffect(() => {
    logEffectRun('sync fetchUserProfileRef');
    fetchUserProfileRef.current = auth.fetchUserProfile;
  }, [auth.fetchUserProfile, logEffectRun]);
  ...
  useEffect(() => {
    logEffectRun('reset race state on selection change');
    resetRaceConditions();
    setTacticalZones([]);
  }, [selectedRaceId, logEffectRun, resetRaceConditions, setTacticalZones]);

  useEffect(() => {
    logEffectRun('document type resolver cleanup setup');
    return () => {
  ...
  useEffect(() => {
    logEffectRun('load user post-race session');
    loadUserPostRaceSession();
  }, [loadUserPostRaceSession, logEffectRun]);

  // Fetch sailor profile ID for strategy cards
  useEffect(() => {
    logEffectRun('fetch sailor profile id');
  ...
  React.useEffect(() => {
    logEffectRun('auth + onboarding guard');
  ...
  }, [ready, signedIn, user?.id, router, isDemoSession, logEffectRun]);
...
function CourseOutlineCard({ groups }: { groups: CourseOutlineGroup[] }) {
  ...
  if (!venueAgentRef.current) {
    console.log('[RacesScreen] Initializing VenueIntelligenceAgent');
  ...
  useEffect(() => {
    logEffectRun('minute ticker interval');
  ...
  useEffect(() => {
    logEffectRun('sync race brief to prep service');
    if (raceBrief && selectedRaceData?.id) {
      updateRaceBrief(raceBrief);
    }
  }, [raceBrief, selectedRaceData?.id, updateRaceBrief, logEffectRun]);
  ...
  useEffect(() => {
    logEffectRun('hydrate rig planner from metadata');
    if (!selectedRaceData) {
  ...
  }, [selectedRaceData, setRegattaAcknowledgements, setRigNotes, setSelectedRigBand, logEffectRun]);

  useEffect(() => {
    logEffectRun('ensure rig preset fallback');
  ...
  }, [rigPresets, selectedRigBand, setSelectedRigBand, logEffectRun]);
  ...
  React.useEffect(() => {
    logEffectRun('cache next race for offline');
    if (nextRace && user) {
      cacheNextRace(nextRace.id, user.id).catch(err =>
        logger.error('Failed to cache race:', err)
      );
    }
  }, [nextRace, user, cacheNextRace, logger, logEffectRun]);
  ...
  React.useEffect(() => {
    ...
    logEffectRun('load cached venue insights from DB');
    setShowInsights(false);
    let cancelled = false;

    loadCachedInsights(currentVenue.id).then((cached) => {
      if (cancelled) {
        return;
      }

      if (cached) {
        setVenueInsights(cached);
        setShowInsights(true);
      } else {
  ...
  }, [currentVenue?.id, loadCachedInsights, logEffectRun]);

  // Trigger AI venue analysis when venue is detected (only if no cached insights)
  React.useEffect(() => {
    logEffectRun('trigger AI venue analysis when needed');
    if (currentVenue && confidence > 0.5 && !venueInsights) {
      handleGetVenueInsights();
    }
  }, [currentVenue, confidence, venueInsights, handleGetVenueInsights, logEffectRun]);
  ...
  React.useEffect(() => {
    logEffectRun('auto select dataset');
  ...
  }, [loading, safeRecentRaces, nextRace, selectedRaceId, setSelectedRaceId, logger, logEffectRun]);

  useEffect(() => {
    logEffectRun('track manual selected race id');
    if (selectedRaceId) {
      autoSelectLastAppliedIdRef.current = selectedRaceId;
    }
  }, [selectedRaceId, logEffectRun]);

  // Handle deep links that request a specific race selection (e.g., after creation)
  useEffect(() => {
    logEffectRun('apply initial route param selection');
  ...
  }, [loading, safeRecentRaces, logEffectRun]);

  useEffect(() => {
    logEffectRun('sync demo selection fallback');
    if (hasRealRaces) {
  ...
  }, [hasRealRaces, loading, selectedDemoRaceId, setSelectedDemoRaceId, logEffectRun]);

  // Scroll to race detail when a race is selected (but not on initial auto-select)
  React.useEffect(() => {
    logEffectRun('scroll to detail after manual selection');
  ...
  }, [selectedRaceId, hasManuallySelected, logEffectRun]);

  // Fetch race detail when selected race changes
  React.useEffect(() => {
    logEffectRun('fetch race detail payload');
    ...
    fetchRaceDetail();
  }, [selectedRaceId, raceDetailReloadKey, logEffectRun]);

  useEffect(() => {
    logEffectRun('load race documents set');
    let isActive = true;
  ...
  }, [selectedRaceData?.id, raceDetailReloadKey, raceDocumentsReloadKey, isDemoSession, logEffectRun]);
...
const raceDocumentsForDisplay = useMemo<RaceDocumentsCardDocument[]>(() => {
  ...
  useEffect(() => {
    logEffectRun('reset fallback position tracking');
    fallbackPositionInitializedRef.current = false;
  }, [fallbackPosition?.latitude, fallbackPosition?.longitude, hasActiveRace, logEffectRun]);
  ...
  useEffect(() => {
    logEffectRun('sync boat draft to store');
    if (typeof boatDraftValue === 'number' && Math.abs(boatDraftValue - currentDraft) > 0.01) {
      setDraft(boatDraftValue);
    }
  }, [boatDraftValue, currentDraft, setDraft, logEffectRun]);

  useEffect(() => {
    logEffectRun('update environment snapshots');
    if (!hasActiveRace) {
      return;
    }
  ...
  useEffect(() => {
    logEffectRun('update course store');
  ...
  }, [hasActiveRace, raceCourseForConsole, updateCourse, logEffectRun]);

  useEffect(() => {
    logEffectRun('generate tactical zones');
    let cancelled = false;

    if (!hasActiveRace || !windSnapshot || !currentSnapshot || !raceCourseForConsole || !venueCenter) {
      setTacticalZones([]);
      return;
    }
    ...
    const run = async () => {
      try {
        const zones = await TacticalZoneGenerator.generateZones({
          wind: {
            speed: windSnapshot.speed,
            direction: windSnapshot.direction,
          } as any,
        ...
        if (!cancelled) {
          logger.error('Failed to generate tactical zones', error);
          setTacticalZones([]);
        }
      }
    };
  ...
  useEffect(() => {
    logEffectRun('seed fallback GPS position');
  ...
  }, [hasActiveRace, fallbackPosition, gpsPosition, logEffectRun, updateRacePosition]);
  ...
  React.useEffect(() => {
    logEffectRun('clear detail data on selection change');
    logger.debug('[races.tsx] 🔄 Selection changed, clearing previous details for ID:', selectedRaceId);
    setSelectedRaceData(null);
    setSelectedRaceMarks([]);
  }, [selectedRaceId, setSelectedRaceData, setSelectedRaceMarks, logEffectRun]);

  // Auto-scroll to center on next race
  useEffect(() => {
    logEffectRun('auto-center next race card on load');
  ...
  }, [nextRace, safeRecentRaces, loading, hasManuallySelected, SCREEN_WIDTH, logEffectRun]);

  // Keep the selected race visible by centering it when selection changes
  useEffect(() => {
    logEffectRun('center selected race card on selection change');
  ...
  }, [selectedRaceId, safeRecentRaces, SCREEN_WIDTH, logEffectRun]);
