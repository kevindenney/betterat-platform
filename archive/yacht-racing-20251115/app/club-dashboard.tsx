...
import { Text } from '@betterat/ui/components/ui/text';
import { useClubs } from '@betterat/core/hooks/useData';
import { useClubMembers } from '@betterat/core/hooks/useClubMembers';
import {
  ClubIntegrationSnapshot,
  ClubMembershipRecord,
  ClubDocumentSnapshot,
  useClubIntegrationSnapshots,
} from '@betterat/core/hooks/useClubIntegrationSnapshots';
import { useClubEntries } from '@betterat/core/hooks/useClubEntries';
import {
  ClubRole,
...
const ClubDashboardScreen = () => {
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      case 'dashboard': {
        const primaryCurrency = entrySummary.currencyBreakdown[0]?.currency || 'USD';
        const regattas = snapshot.regattas;
        const highlightRegatta = regattas[0];

        const quickActions: QuickAction[] = [
          { id: 'new-regatta', label: 'New regatta', icon: '🏁', onPress: () => router.push('/club/event/create?type=regatta') },
          { id: 'upload-doc', label: 'Upload docs', icon: '📄', onPress: () => setActiveView('documents') },
          { id: 'entries', label: 'Entries desk', icon: '💳', onPress: () => setActiveView('entries') },
        ];

        const highlightConfig: HighlightEvent | undefined = highlightRegatta
          ? {
              name: highlightRegatta.name,
              dateRange: formatEventDateRange(highlightRegatta.startDate, highlightRegatta.endDate),
              venue: highlightRegatta.venue || 'Venue TBD',
              info: `${highlightRegatta.eventType || 'Series'} • ${highlightRegatta.status || 'Planning'}`,
              checklist: [
                { label: 'Registration', state: highlightRegatta.registrationOpen ? 'complete' : 'warning' },
                { label: 'Documents', state: highlightRegatta.documents?.length ? 'complete' : 'pending' },
                { label: 'Volunteers', state: (snapshot.insights.volunteerNeeds ?? 0) > 0 ? 'warning' : 'complete' },
              ],
              actions: [
                { label: 'View details', onPress: () => router.push(`/club/event/${highlightRegatta.id}`) },
                { label: 'Plan strategy', variant: 'secondary', onPress: () => router.push('/(tabs)/events') },
              ],
            }
          : undefined;

        const upcomingList: UpcomingEvent[] = regattas.slice(1).map((regatta) => ({
          id: regatta.id,
          name: regatta.name,
          date: formatEventDateRange(regatta.startDate, regatta.endDate),
          venue: regatta.venue || 'Venue TBD',
          badge: regatta.status || undefined,
        }));

        const heroSubtitle = highlightRegatta
          ? `Next up ${highlightRegatta.name} ${formatDistanceToNow(new Date(highlightRegatta.startDate), { addSuffix: true })}`
          : 'Launch regattas, sync documents, and track entries from one workspace.';

        const emptyState = regattas.length
          ? undefined
          : {
              icon: '📅',
              title: 'No regattas connected',
              subtitle: 'Connect RegattaFlow to manage club events and entries.',
