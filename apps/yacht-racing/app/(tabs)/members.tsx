...
import { ClubRole } from '@betterat/core/types/club';
...
type MemberFilterKey = keyof MemberFilterState;
type ToggleableFilterKey = Exclude<MemberFilterKey, 'search'>;
...
const roleOptions: ClubRole[] = ['admin', 'race_officer', 'scorer', 'communications', 'member'];
...
export default function MembersScreen() {
  ...
  const [filters, setFilters] = useState<MemberFilterState>(initialFilters);
  ...
  const toggleFilter = <K extends ToggleableFilterKey>(
    type: K,
    value: MemberFilterState[K][number]
  ) => {
    setFilters((prev) => {
      const currentValues = prev[type] as Array<MemberFilterState[K][number]>;
      const hasValue = currentValues.includes(value);
      const nextValues = (hasValue
        ? currentValues.filter((entry) => entry !== value)
        : [...currentValues, value]) as MemberFilterState[K];
                ...
                <View style={styles.filterChipRow}>
                  {roleOptions.map((option) => (
                    <FilterChip
                      key={option}
                      label={formatLabel(option)}
                      active={filters.role.includes(option as ClubRole)}
