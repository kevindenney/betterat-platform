...
import {
  ...
  useCoachMetrics,
  useCoachResources,
} from '@betterat/core/hooks/useCoachData';
import { useAuth } from '@betterat/core/providers/AuthProvider';
import { CoachDashboard } from '@betterat/ui/components/coaching/CoachDashboard';
...
export default function CoachingHubScreen() {
  ...
  const sessionsToUse = useMemo(() => {
    if (recentSource && recentSource.length > 0) {
      return recentSource;
    }
    if (upcomingSource && upcomingSource.length > 0) {
      return upcomingSource;
    }
    return MOCK_SESSIONS;
  }, [recentSource, upcomingSource]);
